...
callsToInterface = [method | method <- localMethodCalls, all( containingClass <- domain(rangeR(model@containment, {method})), containingClass.scheme == "java+interface")]; 

if(isEmpty(localMethodCalls)) return 100.0;	// If we don't have any method calls, we have pretty loose coupling
if(isEmpty(callsToInterface)) return 0.0;	// If we don't have any method calls to interfaces, we have pretty tight coupling
